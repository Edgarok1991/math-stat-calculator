/**
 * Клиентский расчёт ANOVA (fallback при недоступности API)
 * Используется на Vercel, когда backend не настроен
 */

import type { AnovaResult, AnovaStep } from '@/types/calculator';
...
const F_TABLE: Record<number, Record<number, Record<number, number>>> = {
  0.01: {
...
function getFCritical(df1: number, df2: number, alpha: number): number {
  const a = alpha as keyof typeof F_TABLE;
  const df1c = Math.min(Math.max(df1, 1), 5);
  const keys = [1, 2, 3, 4, 5, 8, 10, 12, 15, 20, 24, 30, 40, 60, 120];
  ...
  return F_TABLE[a]?.[df1c]?.[k] ?? F_TABLE[0.05]?.[df1c]?.[12] ?? 3.89;
}
...
function calculatePValue(fStatistic: number, df1: number, df2: number): number {
  if (fStatistic <= 0) return 1;
  if (fStatistic > 50) return 0.00001;
  try {
    const x = df2 / (df2 + df1 * fStatistic);
    const p = 1 - regularizedIncompleteBeta(x, df2 / 2, df1 / 2);
...
export function calculateAnovaClient(groups: number[][], alpha: number): AnovaResult {
  ...
  const totalCount = groupSizes.reduce((a, b) => a + b, 0);
  const grandMean = groups.flat().reduce((a, b) => a + b, 0) / totalCount;

  const ssb = groups.reduce((s, g, i) => s + g.length * (groupMeans[i] - grandMean) ** 2, 0);
  const ssw = groups.reduce((s, g, i) => s + g.reduce((sg, x) => sg + (x - groupMeans[i]) ** 2, 0), 0);
  const sst = ssb + ssw;
  ...
  const criticalValue = getFCritical(dfBetween, dfWithin, alpha);
  const pValue = calculatePValue(fStatistic, dfBetween, dfWithin);
  ...
  const totalSum = groups.flat().reduce((a, b) => a + b, 0);
  ...
  const steps: AnovaStep[] = [
    {
      ...
      values: { totalSum, totalCount, grandMean },
    },
    {
      step: 2,
      title: 'Средние по группам',
      description: 'Вычисляем среднее для каждой группы.',
      values: Object.fromEntries(groupMeans.map((m, i) => [`Группа ${i + 1}`, m])),
    },
    {
      step: 3,
